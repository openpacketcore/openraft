...
impl ExampleClient {
    ...
    pub async fn write(
        &self,
        req: &Request,
    ) -> Result<typ::ClientWriteResponse, Box<typ::RPCError<typ::ClientWriteError>>> {
    ...
    pub async fn read(&self, req: &String) -> Result<String, Box<typ::RPCError>> {
    ...
    pub async fn consistent_read(&self, req: &String) -> Result<String, Box<typ::RPCError<typ::CheckIsLeaderError>>> {
    ...
    pub async fn init(&self) -> Result<(), Box<typ::RPCError<typ::InitializeError>>> {
    ...
    /// See [`RaftMetrics`].
    pub async fn metrics(&self) -> Result<RaftMetrics<NodeId, BasicNode>, Box<typ::RPCError>> {
    ...
    async fn do_send_rpc_to_leader<Req, Resp, Err>(
        ...
        uri: &str,
        req: Option<&Req>,
    ) -> Result<Resp, Box<typ::RPCError<Err>>>
    where
        ...
        Err: std::error::Error + Serialize + DeserializeOwned,
    {
        ...
        let resp = match res {
            ...
            Err(timeout_err) => {
                ...
                return Err(Box::new(typ::RPCError::Network(NetworkError::new(&timeout_err))));
            }
        };
        ...
        res.map_err(|e| Box::new(typ::RPCError::RemoteError(RemoteError::new(leader_id, e))))
    }
        ...
        Err: std::error::Error + Serialize + DeserializeOwned + TryAsRef<typ::ForwardToLeader> + Clone,
    {
        ...
        loop {
            let res: Result<Resp, Box<typ::RPCError<Err>>> = self.do_send_rpc_to_leader(uri, req).await;
