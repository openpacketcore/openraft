...
#[test]
fn test_failed_targets_do_not_repin_each_others_pending_purge() {
    let mut eng = snapshot_tail_engine();
    let membership = Membership::new(vec![btreeset! {1,2,3}], None);
    eng.state.membership_state = MembershipState::new(
        EffectiveMembership::new_arc(Some(log_id(1, 0, 1)), membership.clone()),
        EffectiveMembership::new_arc(Some(log_id(1, 0, 1)), membership),
    );
    eng.testing_new_leader();
    for target in [2, 3] {
        let progress = eng.leader.as_mut().unwrap().progress.get_mut(&target).unwrap();
        progress.searching_end = 6;
        progress.curr_inflight_id = 7;
        progress.inflight = Inflight::logs(Some(log_id(1, 0, 2)), Some(log_id(1, 0, 5))).with_id(7);
    }
    eng.output.take_commands();

    eng.replication_handler().update_progress(
        2,
        RequestId::new_append_entries(7),
        Err("first target unavailable".to_string()),
    );
    assert_eq!(
        Some(&log_id(1, 0, 2)),
        eng.state.last_purged_log_id(),
        "the other transfer still owns its logs"
    );
    assert_eq!(
        vec![Command::Replicate {
            target: 2,
        ...
        eng.output.take_commands(),
        "a failed transfer must not repin a range pending purge behind another owner",
    );

    eng.replication_handler().update_progress(
        3,
        RequestId::new_append_entries(7),
        Err("second target unavailable".to_string()),
    );
    assert_eq!(Some(&log_id(1, 0, 10)), eng.state.last_purged_log_id());
    assert_eq!(
        vec![Command::PurgeLog { upto: log_id(1, 0, 10) }, Command::Replicate {
        ...
        eng.output.take_commands(),
        "the last failed owner releases the pending purge before either retry can retain it",
    );
    assert_eq!(None, eng.state.committed(), "failed replication grants no commitment");
}
